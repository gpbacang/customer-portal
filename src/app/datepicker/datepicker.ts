...
export class DateTime {
  ...
  static parseDate(dateStr: string, parseFormat?: string, dateFormat?: string): Date {
    if (typeof moment === 'undefined') {
      ...
      return DateTime.parseFromDefaultFormat(dateStr);
  ...
  private static addDSTOffset(dateStr): string {
    let date = DateTime.parseFromDefaultFormat(dateStr);
  ...
  getMonthData(year: number, month: number): any {
    ...
    let leadingDays = (dayOfWeek - DateTime.firstDayOfWeek + 7) % 7 || 7;
    let trailingDays = DateTime.days.slice(0, 6 * 7 - (leadingDays + daysInMonth));
    ...
    let firstWeekNumber = DateTime.getWeekNumber(firstDay);
    ...
    let localizedDaysOfWeek =
      DateTime.daysOfWeek
        .concat(DateTime.daysOfWeek)
        .splice(DateTime.firstDayOfWeek, 7);
        

    let monthData = {
      year: year,
      month: month,
      weekends: DateTime.weekends,
      firstDayOfWeek: DateTime.firstDayOfWeek,
      fullName: DateTime.months[month].fullName,
      shortName: DateTime.months[month].shortName,
      localizedDaysOfWeek: localizedDaysOfWeek,
      days: DateTime.days.slice(0, daysInMonth),
      leadingDays: DateTime.days.slice(-leadingDays - (31 - daysInLastMonth), daysInLastMonth),
